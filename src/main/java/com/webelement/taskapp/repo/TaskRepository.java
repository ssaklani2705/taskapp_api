...
public interface TaskRepository extends JpaRepository<TaskEntity, Integer> {
	...
	@Query("SELECT DISTINCT c.clientId, c.name "
	        ...
	        + "    )"
	        + ")")
	List<Object[]> findDashboardClients(
...
//	@Query("SELECT new com.webelement.taskapp.dto.TaskDetailsDTO(t.taskId, c.name, t.date,tc.duedatetime, tc.name, COALESCE(u.firstName, '0'), t.priority, t.status, t.title,t.taskStatus,t.assignedTo,t.addedBy) "
//			+ "FROM TaskEntity t " + "LEFT JOIN ClientEntity c ON c.clientId = t.clientId "
//			+ "LEFT JOIN TaskCategoryEntity tc ON tc.taskcategoryId = t.taskCategoryId "
...
//			+ "AND (:assignedTo = -1 OR t.assignedTo = :assignedTo) " + "AND (:priority = 0 OR t.priority = :priority) "
...
//			+ "ORDER BY t.status ASC,  c.name ASC")
	...
	Page<TaskDetailsDTO> findTaskDetails(
	        ...
	        @Param("isAdmin") String isAdmin,
	        @Param("userId") Integer userId,
	        @Param("taskStatusId") Integer taskStatusId,
	        @Param("loginType") String loginType
	);
...
//	List<TaskEntity> findDashboardTasks(@Param("userId") Integer userId,@Param("isAdmin")  String isAdmin);
	...
	@Query("SELECT t "
	        + "FROM TaskEntity t "
	        + "WHERE t.status = 1 AND t.clientId = :selectedClientId "
	        + "AND ("
