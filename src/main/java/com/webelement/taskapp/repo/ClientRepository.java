...
public interface ClientRepository extends JpaRepository<ClientEntity, Integer> {
	...
	@Query("SELECT new com.webelement.taskapp.dto.ClientDTO("
			...
			+ "c.regdate, c.moddate, c.taxFlag, c.location, c.planId, p.name) "
			+ " FROM ClientEntity c LEFT JOIN StateEntity s "
			+ "ON c.stateId = s.stateId LEFT JOIN UserLoginEntity u ON c.managerId = u.userId "
			+ "LEFT JOIN PlanEntity p ON c.planId = p.planId WHERE c.clientId = :clientId")
