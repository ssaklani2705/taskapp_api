...
//			+ "c.clientId, c.name, c.code, c.pan, c.status, c.gstFlag, c.gstNo, c.stateId, s.name, c.addressLine1, c.addressLine2, "
//			+ "c.city, c.pincode, c.contactName, c.contactEmail, c.emails, c.startDate, c.monthlyCharge, c.outstanding, "
//			+ "c.name1, c.emailId1, c.name2, c.emailId2, c.name3, c.emailId3, c.managerId, u.firstName, c.userId, c.regdate, "
//			+ "c.moddate, c.taxFlag, c.location) "
...
//			+ "AND (:status = 0 OR c.taxFlag = :status) AND (:managerId = 0 OR c.managerId = :managerId) "
//			+ "AND (:stateId = 0 OR c.stateId = :stateId) AND (:clientName IS NULL OR :clientName = '' OR LOWER(c.name) LIKE LOWER(CONCAT('%', :clientName, '%'))) "
//			+ "AND (:clientCode IS NULL OR :clientCode = '' OR LOWER(c.code) LIKE LOWER(CONCAT('%', :clientCode, '%'))) "
//			+ "AND (:contactName IS NULL OR :contactName = '' OR LOWER(c.contactName) LIKE LOWER(CONCAT('%', :contactName, '%'))) "
//			+ "AND (:contactEmail IS NULL OR :contactEmail = '' OR LOWER(c.contactEmail) LIKE LOWER(CONCAT('%', :contactEmail, '%'))) "
//			+ "AND (:search IS NULL OR :search = '' OR LOWER(c.name) LIKE LOWER(CONCAT('%', :search, '%')) OR LOWER(c.code) LIKE LOWER(CONCAT('%', :search, '%')) "
//			+ "OR LOWER(c.contactName) LIKE LOWER(CONCAT('%', :search, '%')) OR LOWER(c.contactEmail) LIKE LOWER(CONCAT('%', :search, '%')))")
